...
describe('Filter Feature', () => {
	describe('Controller Tests', () => {
		describe('GET /api/filter/posts', () => {
			...
			it('should return 400 when start date format is invalid', async () => {
				const { response, body } = await helpers.request('get', '/api/filter/posts?start=2025-13-01&end=2025-10-01');
				assert.equal(response.statusCode, 400);
				assert.strictEqual(body.success, false);
			...
			it('should return 400 when end date format is invalid', async () => {
				const { response, body } = await helpers.request('get', '/api/filter/posts?start=2025-09-01&end=2025-15-32');
				assert.equal(response.statusCode, 400);
				assert.strictEqual(body.success, false);
				assert.strictEqual(body.error, 'invalid_date');
			});
	...
	describe('Posts Service Tests', () => {
		...
		it('should handle start time equal to end time', async () => {
			const time = Date.now();
			
			try {
				await Posts.getPostsByTimeRange(1, time, time, 0, 10, -1);
				assert.fail('Should have thrown an error for equal start and end times');
			} catch (error) {
				assert(error.message.includes('invalid-time-range'));
			}
		});

		it('should handle start time greater than end time', async () => {
			const now = Date.now();
			const earlier = now - 1000;
			
			try {
				await Posts.getPostsByTimeRange(1, now, earlier, 0, 10, -1);
				assert.fail('Should have thrown an error for start > end');
			} catch (error) {
				assert(error.message.includes('invalid-time-range'));
			}
		});
		...
		it('should handle null/undefined start time', async () => {
			try {
				await Posts.getPostsByTimeRange(1, null, Date.now(), 0, 10, -1);
				assert.fail('Should have thrown an error for null start time');
			} catch (error) {
				assert(error.message.includes('invalid-time-format'));
			}
		});

		it('should handle null/undefined end time', async () => {
			try {
				await Posts.getPostsByTimeRange(1, Date.now(), null, 0, 10, -1);
				assert.fail('Should have thrown an error for null end time');
			} catch (error) {
				assert(error.message.includes('invalid-time-format'));
			}
		});
		...
		it('should handle malformed date string', async () => {
			try {
				await Posts.getPostsByTimeRange(1, 'not-a-date', Date.now(), 0, 10, -1);
				assert.fail('Should have thrown an error for malformed date');
			} catch (error) {
				assert(error.message.includes('invalid-date-format'));
			}
		});
